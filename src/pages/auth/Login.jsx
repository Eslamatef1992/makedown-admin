...
export default function Login() {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-linen-50 px-4">
      <div className="pointer-events-none absolute -start-40 bottom-0 h-[620px] w-[620px] rounded-full bg-carissma-100/60 blur-3xl" />
      <div className="pointer-events-none absolute -end-20 top-0 h-[420px] w-[420px] rounded-full bg-saffron-100/60 blur-3xl" />

      <button
        onClick={toggleLanguage}
        className="absolute end-6 top-6 rounded-full border border-linen-300 bg-white/80 px-4 py-2 text-sm font-medium text-espresso-700 hover:bg-white"
      >
        {t('topbar.language')}
      </button>
      ...
      <div className="relative w-full max-w-md rounded-3xl bg-white/90 p-8 shadow-xl backdrop-blur">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="text-2xl font-semibold text-espresso-900">{t('login.welcomeBack')}</h1>
          <p className="mt-1 text-sm text-espresso-500">{t('login.subtitle')}</p>
        </div>

        <form onSubmit={onSubmit} className="space-y-4">
          <label className="block">
            <span className="mb-2 block text-sm font-medium text-espresso-800">{t('login.email')}</span>
            <input
              type="email"
              name="email"
          ...
          <label className="block">
            <span className="mb-2 block text-sm font-medium text-espresso-800">{t('login.password')}</span>
            <input
              type="password"
              name="password"
              placeholder="••••••••"
